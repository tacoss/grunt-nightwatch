...
module.exports = function(grunt) {
  ...
  var argv = _.pick(grunt.cli.options, 'retries', 'suiteRetries', 'tag', 'test', 'filter', 'verbose', 'group', 'skipgroup');
  ...
  return function(group, options, settings, doneCallback) {
    ...
    function spawnNightwatch(done) {
      ...
      $.verbose.ok('Settings (' + group.join(', ') + ')');
      $.verbose.writeln(JSON.stringify(_.omit(settings, 'test_settings')));

      $.verbose.ok('Test settings');
      $.verbose.writeln(JSON.stringify(settings.test_settings));
    ...
    if (options.standalone) {
      if (options.jar_path && $.exists(options.jar_path)) {
        ...
        if (!grunt.cli.options.force && $.exists(jar)) {
